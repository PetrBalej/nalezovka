...
<?php if ($this->router->fetch_class() == "detail") { ?>
    <!-- geonames -->
    <script>
        $.getJSON("http://api.geonames.org/findNearbyPlaceNameJSON?formatted=true&lat=<?php echo $nalez['decimalLatitude']; ?>&lng=<?php echo $nalez['decimalLongitude']; ?>&username=petrb&style=full", function (data) {
            // success
            $(document).ready(
                    function () {
