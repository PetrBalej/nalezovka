...
class Jezera extends MY_Controller
{
    ...
    public function index($id = null)
    {
        ...
        $sql_add = preg_replace("/\sFROM\s/m", ", ST_AsText(ST_Centroid(ST_Envelope(ST_GeomFromText( CONCAT( 'GEOMETRYCOLLECTION(', GROUP_CONCAT(ST_AsText(coordinates)), ')' ) )))) AS center FROM ", trim_sql_comments($this->public_sql[10]), 1);
